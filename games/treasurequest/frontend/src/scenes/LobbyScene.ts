...
export default class LobbyScene extends Phaser.Scene {
  ...
  private playerName = `Explorer-${Math.floor(Math.random() * 1000)}`;
  ...
  create(): void {
    ...
    this.add
      .text(600, 195, 'Treasure Quest rooms', {
        fontSize: '13px',
        color: '#aaaaaa',
        fontFamily: 'monospace',
      })
    ...
    joinButton.on('pointerout', () => joinButton.setColor('#ffd166'));
    ...
    try {
      ...
      this.statusText.setText(
        onlineHosts.length === 0
          ? 'No LAN hosts found — start the host JAR on this network'
  ...
  private async loadRoomsForSelectedHost(showLoadingStatus: boolean): Promise<void> {
    ...
    try {
      ...
      this.hostRooms = QUEST_ONLY
        ? allRooms.filter((room) => room.gamePluginId === QUEST_PLUGIN_ID)
        : allRooms;
      if (QUEST_ONLY && this.hostRooms.length === 1) {
  ...
  private renderRoomList(message?: string): void {
    ...
    if (this.hostRooms.length === 0) {
      const hint = QUEST_ONLY
        ? 'No Treasure Quest room on this host.\nRestart the server JAR (needs room "quest").'
        : this.loadingRooms
          ? 'Loading rooms…'
          : 'No rooms on this host';
      const empty = this.add
        .text(600, 240, hint, {
          fontSize: '14px',
          color: '#666666',
