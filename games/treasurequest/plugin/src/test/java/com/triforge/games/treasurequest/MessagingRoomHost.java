...
/** Room host stub that records {@code sendTo} messages for integration tests. */
final class MessagingRoomHost implements RoomHost {

    private final String roomId;
    private long tick;
    private long nextPlayerId = 1L;
    private final List<GameMessage> messagesToPlayer1 = new ArrayList<>();

    MessagingRoomHost(String roomId) {
        this.roomId = roomId;
    }
    ...
    List<GameMessage> messagesToPlayer(long playerId) {
        if (playerId == 1L) {
            return messagesToPlayer1;
        }
        return List.of();
    }

    void clearMessages() {
        messagesToPlayer1.clear();
    }
